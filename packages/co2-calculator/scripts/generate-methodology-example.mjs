...
function run(gridKey, utilization) {
...
// Little's Law value), not the instantaneous time-of-day value — §3.2d.
...
const SHARED_UTILIZATION = 0.70; // a well-run shared node (§3.2c)

console.log("### §6.2 worked example (generated from dist/index.js)");
console.log("");
console.log(`Model: ${model.displayName}, ${model.defaultInputTokens} in / ${model.defaultOutputTokens} out, Sweden, 14:00, B300, shared (70% utilization), caching on.`);
console.log("");
...
console.log(`| Utilization | shared node, §3.2c | ${SHARED_UTILIZATION} |`);
...
const r = run("sweden", SHARED_UTILIZATION);
const c = r.components;
console.log(`| GPU compute CO₂ | incremental energy × ${intensity.toFixed(1)} / ${dayAvgConc} | ${fmt(c.gpuOperational.co2Grams)} |`);
console.log(`| GPU idle CO₂ | idle energy × ${intensity.toFixed(1)} / ${dayAvgConc} / ${SHARED_UTILIZATION} | ${fmt(c.gpuIdle.co2Grams)} |`);
console.log(`| Server CO₂ | chassis energy × ${intensity.toFixed(1)} / ${dayAvgConc} / ${SHARED_UTILIZATION} | ${fmt(c.serverOperational.co2Grams)} |`);
...
console.log("\n### §6.3 utilization comparison (generated)");
console.log("");
console.log("Same model, request, grid and hour — only the utilization changes (§3.2c).");
console.log("");
console.log("| Deployment | Utilization | Total CO₂e | vs shared |");
console.log("|------------|-------------|-----------:|----------:|");
const shared = r.totalCO2Grams;
for (const [label, u] of [["on-prem", 0.10], ["shared (Berget)", 0.70], ["hyperscaler", 0.90]]) {
  const rr = run("sweden", u);
  console.log(`| ${label} | ${(u * 100).toFixed(0)}% | ${fmt(rr.totalCO2Grams)} | ${(rr.totalCO2Grams / shared).toFixed(2)}× |`);
}
...
console.log("|-----------|---------------------------|----------------------------------|");
const us = run("usa", SHARED_UTILIZATION);
