...
**This is why the model choice is the single biggest lever most teams control.** On our own infrastructure, the same short question answered by a large MoE frontier model (Kimi K3, 2.8T) comes out at roughly **180 mg CO₂**, where a specialised model that handles the task just as well (Gemma 4 31B) comes out at **21 mg** — about a **9×** gap, on the same clean Swedish grid. A cleaner grid narrows the operational part of that gap, but it cannot close it: most of the difference is the extra hardware the larger model ties up. The frontier model is the right tool for some tasks — but not all of them, and reaching for it by default is a choice with a measurable cost.
