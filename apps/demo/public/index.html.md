...
Size sets the floor: a model must fit in GPU memory (weights, plus ~20% for the KV cache), so a trillion-parameter model occupies several GPUs at once while a small quantised one fits on a single card. Context length and cache then stretch or shrink the time: long prompts mean more tokens and a growing KV cache, while a cached prefix lets the model skip most of that work. And how many other queries share the node decides how the fixed cost of the servers is split — though as §3 shows, that sharing is set by who runs the hardware, not by the moment.

**The size of that lever is easy to underestimate.** On our own infrastructure, the same short question answered by a large MoE frontier model (Kimi K3, 2.8T) comes out at roughly **370 mg CO₂**, where a specialised model that handles the task just as well (Gemma 4 31B) comes out at **42 mg** — about a **9×** gap, on the same clean Swedish grid. A cleaner grid narrows the operational part of that gap, but it cannot close it: most of the difference is the extra hardware the larger model ties up. The frontier model is the right tool for some tasks — but not all of them, and reaching for it by default is a choice with a measurable cost.
...
And underneath it all, the meter never fully stops: even an idle GPU keeps drawing a standby baseline — our own DCGM measurements put it at roughly 236 W per card on a flagship B300 node at 0% load; the card never drops into a low-power idle/suspend mode. That standby cost is part of the fixed cost amortised over the day, shared across whoever uses the node — which is why a busy node wastes so much less than an idle one.
...
The same model on the same grid can still emit very differently depending on one more choice: how well the hardware it runs on is used. Run it on your own server, share a node with others (as with Berget AI), or ride a hyperscaler — the difference comes down to a single mechanism, **utilization: how many requests the node's fixed cost is divided across.**

**This is the quiet case for shared infrastructure over a server of your own.** Run a model on an on-prem box and you own the GPU for the whole month — whether it's busy or not. Its servers, cooling and networking draw power around the clock, and its manufacturing footprint is amortised over its life whether or not you use it. A node bought for peak but mostly waiting spreads those fixed costs over few requests, so each one bears more. On a well-used shared node those same costs are divided across many more requests — which is why a busy shared GPU almost always beats a private one that mostly waits.
...
**One axis: how well the hardware is used.** The same model, the same request, the same grid — the only thing that changes is how well the hardware is used. Every fixed cost — the server's standby draw, the chassis, the cooling, and the GPU's own manufacturing footprint — is spent whether the node is busy or idle, so the only question that matters is how many requests it is divided across. A node bought for peak but mostly waiting (your own server, at perhaps 10% utilization) spreads that fixed cost over few requests, so each one bears more. A well-run shared node stays busier; a hyperscale fleet, with massive demand and sophisticated scheduling, keeps its GPUs hottest of all. The hardware's embodied carbon is amortised over its five-year life wherever it sits — utilization changes how many requests that cost, and the standby draw, is divided among. On our own infrastructure the same request comes out at roughly **70 mg** on a mostly-idle private server (10% utilization), **43 mg** on a well-run shared node (70%), and **42 mg** on a hyperscale fleet (90%) — the busier the node, the smaller each request's share of the fixed cost.

> **Method — utilization.** The hardware's embodied carbon is amortised over its 5-year lifetime at a fixed rate, WHEREVER the node sits — it does not depend on utilization at a given concurrency, and it is shared across the day-average concurrency (the batching mechanism works the same everywhere). The fixed ENERGY costs — the GPU idle baseline and the server chassis — are burned around the clock whether the node is serving or not, so their per-request cost scales with 1/utilization: a node idle 90% of the time charges each request roughly 7× more standby than one idle 30% of the time. Utilization anchors: your own server ~10%, a well-run shared node ~70%, a hyperscale fleet ~90%. An idle GPU is NOT in a deep sleep: our DCGM measurements show ~236 W per B300 card at 0% load (measured over 7 days on the production node); that standby draw is part of the fixed cost amortised over the day. Source: Fu et al. (2024), LLMCO2, arXiv:2410.02950.
