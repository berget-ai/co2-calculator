...
function Container({ children }: { children: React.ReactNode }) {
  return <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem' }}>{children}</div>;
}
...
export function CO2Calculator() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', color: '#E5DDD5', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <Container>
        <Stack gap={8} style={{ padding: '2rem 0' }}>
          <h1 style={{ color: '#52B788', margin: 0, fontSize: '2rem', fontWeight: 700 }}>CO₂ Calculator</h1>
          <p style={{ margin: 0, opacity: 0.7 }}>Estimate the carbon footprint of AI inference</p>
          
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
            {/* Controls */}
            <Stack gap={6}>
              <div style={{ background: '#1a1a1a', borderRadius: '12px', padding: '1.5rem', border: '1px solid rgba(255,255,255,0.1)' }}>
                <h2 style={{ marginTop: 0, color: '#52B788', fontSize: '1.125rem', marginBottom: '1rem' }}>Model & Workload</h2>
              ...
              <div style={{ background: '#1a1a1a', borderRadius: '12px', padding: '1.5rem', border: '1px solid rgba(255,255,255,0.1)' }}>
                <h2 style={{ marginTop: 0, color: '#52B788', fontSize: '1.125rem', marginBottom: '1rem' }}>Infrastructure</h2>
            ...
            <div>
              {result && comparisons && (
                <div style={{ background: '#1a1a1a', borderRadius: '12px', padding: '1.5rem', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <h2 style={{ marginTop: 0, color: '#52B788', fontSize: '1.125rem', marginBottom: '1rem' }}>Carbon Impact</h2>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                    <div style={{ textAlign: 'center', padding: '1rem', background: '#0a0a0a', borderRadius: '8px' }}>
                      <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#52B788' }}>{result.totalCO2Grams.toFixed(4)}</div>
                      <div style={{ fontSize: '0.75rem', opacity: 0.6 }}>g CO₂e</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '1rem', background: '#0a0a0a', borderRadius: '8px' }}>
                      <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#52B788' }}>{responseTime.toFixed(1)}s</div>
                      <div style={{ fontSize: '0.75rem', opacity: 0.6 }}>GPU time</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '1rem', background: '#0a0a0a', borderRadius: '8px' }}>
                      <div style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#52B788' }}>{deploymentGrid.name}</div>
